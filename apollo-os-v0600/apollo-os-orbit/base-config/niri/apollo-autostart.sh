#!/bin/bash
# Apollo OS Autostart for Niri v1.0.2
...
# Idle management with hyprlock (only if not already running)
...
# Start specialized monitors (only if not already running)
# These provide TTS notifications for system events

# Power/Battery Monitor
if [ -f "$HOME/.local/bin/apollo-os-power-monitor.sh" ] && ! pgrep -f "apollo-os-power-monitor" >/dev/null; then
    "$HOME/.local/bin/apollo-os-power-monitor.sh" &
fi

# Network Monitor
if [ -f "$HOME/.local/bin/apollo-os-network-monitor.sh" ] && ! pgrep -f "apollo-os-network-monitor" >/dev/null; then
    "$HOME/.local/bin/apollo-os-network-monitor.sh" &
fi

# Sleep/Wake Monitor
if [ -f "$HOME/.local/bin/apollo-os-sleep-monitor.sh" ] && ! pgrep -f "apollo-os-sleep-monitor" >/dev/null; then
    "$HOME/.local/bin/apollo-os-sleep-monitor.sh" &
fi
