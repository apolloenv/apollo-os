...
#####################################################################
# Apollo OS - Master Installer v1.0.1
...
print_banner() {
    ...
    echo -e "${NC}"
    echo -e "${MAGENTA}═══════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}  Apollo OS Installer v1.0.1${NC}"
...
EOF'
    ...
    if ! command -v winboat &>/dev/null; then
        ...
        WINBOAT_RPM="/tmp/winboat-${WINBOAT_VERSION#v}.x86_64.rpm"
...
PORTALEOF
    ...
    cp "$SCRIPT_DIR/config-data/niri/config-classic.kdl" "$HOME/.config/niri/config-classic.kdl" || warn "Classic config deployment failed"
    cp "$SCRIPT_DIR/config-data/niri/config-developer.kdl" "$HOME/.config/niri/config-developer.kdl" || warn "Developer config deployment failed"
    cp "$SCRIPT_DIR/config-data/niri/config-modern.kdl" "$HOME/.config/niri/config-modern.kdl" || warn "Modern config deployment failed"
    cp "$SCRIPT_DIR/config-data/niri/config-orbit.kdl" "$HOME/.config/niri/config-orbit.kdl" || warn "Orbit config deployment failed"
    cp "$SCRIPT_DIR/config-data/niri/config-professional.kdl" "$HOME/.config/niri/config-professional.kdl" || warn "Professional config deployment failed"
    ...
    cp "$SCRIPT_DIR/config-data/waybar/config-niri-classic" "$HOME/.config/waybar/config-niri-classic" || warn "Waybar classic config failed"
    cp "$SCRIPT_DIR/config-data/waybar/config-niri-developer" "$HOME/.config/waybar/config-niri-developer" || warn "Waybar developer config failed"
    cp "$SCRIPT_DIR/config-data/waybar/config-niri-modern" "$HOME/.config/waybar/config-niri-modern" || warn "Waybar modern config failed"
    cp "$SCRIPT_DIR/config-data/waybar/config-niri-orbit" "$HOME/.config/waybar/config-niri-orbit" || warn "Waybar orbit config failed"
    cp "$SCRIPT_DIR/config-data/waybar/config-niri-professional" "$HOME/.config/waybar/config-niri-professional" || warn "Waybar professional config failed"
    cp "$SCRIPT_DIR/config-data/waybar/style.css" "$HOME/.config/waybar/style.css" || warn "Waybar style deployment failed"
    cp "$SCRIPT_DIR/config-data/waybar/style-classic.css" "$HOME/.config/waybar/style-classic.css" || warn "Waybar classic style failed"
    cp "$SCRIPT_DIR/config-data/waybar/style-developer.css" "$HOME/.config/waybar/style-developer.css" || warn "Waybar developer style failed"
    cp "$SCRIPT_DIR/config-data/waybar/style-modern.css" "$HOME/.config/waybar/style-modern.css" || warn "Waybar modern style failed"
    cp "$SCRIPT_DIR/config-data/waybar/style-orbit.css" "$HOME/.config/waybar/style-orbit.css" || warn "Waybar orbit style failed"
    cp "$SCRIPT_DIR/config-data/waybar/style-professional.css" "$HOME/.config/waybar/style-professional.css" || warn "Waybar professional style failed"
...
install_flatpak_apps() {
    ...
    flatpak remote-add --if-not-exists flathub https://dl.flathub.org/repo/flathub.flatpakrepo 2>/dev/null || log "Flathub already configured"
    ...
    local flatpak_apps=(
        ...
        "be.alexandervanhee.gradia"          # Image gradients
        "io.gitlab.theevilskeleton.Upscaler" # Image upscaler
        
        # Development
        "org.apache.netbeans"                # NetBeans IDE
        "org.eclipse.Java"                   # Eclipse IDE
        "com.vscodium.codium"                # VSCodium (FOSS VS Code)
        "org.geany.Geany"                    # Lightweight IDE
        "org.gnome.meld"                     # Diff viewer
        "org.gnome.Devhelp"                  # API documentation
        ...
        "com.github.xournalpp.xournalpp"     # PDF annotator
        "com.adobe.Reader"                   # Adobe Reader
        "app.drey.Biblioteca"                # Document manager
    ...
    for app in "${flatpak_apps[@]}"; do
        ...
        if timeout 300 flatpak install -y --noninteractive flathub "$app" 2>&1 | grep -q "error"; then
