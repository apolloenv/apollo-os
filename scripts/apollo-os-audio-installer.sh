...
download_voice_model() {
    ...
    # Download LUNA model (en_GB-jenny_dioco-medium)
    local base_url="https://github.com/rhasspy/piper/releases/download/v1.2.0"
    local model_name="en_GB-jenny_dioco-medium"
    
    log "Downloading model file..."
    wget -q --show-progress \
        -O "$VOICE_DIR/luna.onnx" \
        "$base_url/$model_name.onnx" || error "Failed to download voice model"
    
    log "Downloading model config..."
    wget -q --show-progress \
        -O "$VOICE_DIR/luna.onnx.json" \
        "$base_url/$model_name.onnx.json" || error "Failed to download model config"
