...
#####################################################################
# Apollo OS - Master Installer v3.2.0
...
#
# v3.2.0 Changes:
# - 16 Visual Modes (Classic, Developer, Enterprise, i3, Minimal, Modern, macOS, etc.)
# - macOS Sequoia inspired theme with dock
# - Voice Input with Whisper.cpp (Super+V and Right Ctrl Push-to-Talk)
# - Wake Word "Apollo" voice assistant (Vosk)
# - Hyprlock integration for Niri (Super+L)
# - KDE System Settings for Niri
# - Optional Flatpak applications installation
# - Mako notification theming per visual mode
#
# v3.1.0 Changes:
...
BANNER
    echo -e "${NC}"
    echo -e "${MAGENTA}═══════════════════════════════════════════════════════${NC}"
    echo -e "${CYAN}  Apollo OS Installer v3.2.0${NC}"
...
PORTALEOF
    ...
    if [[ "$INSTALL_NIRI" == true ]]; then
        ...
        # Copy main config
        cp "$SCRIPT_DIR/config-data/niri/config.kdl" "$HOME/.config/niri/config.kdl" || warn "Main Apollo OS Orbit config deployment failed"

        # Copy all visual mode configs from visual-modes directory
        log "Deploying all 16 Visual Mode configurations..."
        if [ -d "$SCRIPT_DIR/visual-modes/niri" ]; then
            cp "$SCRIPT_DIR/visual-modes/niri/"*.kdl "$HOME/.config/niri/" || warn "Visual modes niri config deployment failed"
        else
            # Fallback to config-data if visual-modes not found
            cp "$SCRIPT_DIR/config-data/niri/config-classic.kdl" "$HOME/.config/niri/" 2>/dev/null
            cp "$SCRIPT_DIR/config-data/niri/config-developer.kdl" "$HOME/.config/niri/" 2>/dev/null
            cp "$SCRIPT_DIR/config-data/niri/config-modern.kdl" "$HOME/.config/niri/" 2>/dev/null
            cp "$SCRIPT_DIR/config-data/niri/config-orbit.kdl" "$HOME/.config/niri/" 2>/dev/null
            cp "$SCRIPT_DIR/config-data/niri/config-professional.kdl" "$HOME/.config/niri/" 2>/dev/null
        fi

        # Copy toggle scripts
        cp "$SCRIPT_DIR/config-data/niri/apollo-autostart.sh" "$HOME/.config/niri/" || warn "Apollo OS Orbit autostart deployment failed"
        if [ -d "$SCRIPT_DIR/visual-modes/scripts" ]; then
            cp "$SCRIPT_DIR/visual-modes/scripts/toggle-"*.sh "$HOME/.config/niri/" 2>/dev/null
        else
            cp "$SCRIPT_DIR/config-data/niri/toggle-center.sh" "$HOME/.config/niri/" 2>/dev/null
        fi
    ...
    if [[ "$INSTALL_HYPRLAND" == true ]]; then
        ...
        local HYPR_SRC="$SCRIPT_DIR/apollo-os-professional/dots/.config"
        ...
        local SHARE_SRC="$SCRIPT_DIR/apollo-os-professional/dots/.local/share"
    ...
    cp "$SCRIPT_DIR/config-data/waybar/config-niri" "$HOME/.config/waybar/config-niri" || warn "Waybar config deployment failed"
    cp "$SCRIPT_DIR/config-data/waybar/style.css" "$HOME/.config/waybar/style.css" || warn "Waybar style deployment failed"

    # Copy all visual mode waybar configs from visual-modes directory
    log "Deploying all 16 Visual Mode Waybar configurations..."
    if [ -d "$SCRIPT_DIR/visual-modes/waybar/configs" ]; then
        cp "$SCRIPT_DIR/visual-modes/waybar/configs/"* "$HOME/.config/waybar/" || warn "Visual modes waybar config deployment failed"
    fi
    if [ -d "$SCRIPT_DIR/visual-modes/waybar/styles" ]; then
        cp "$SCRIPT_DIR/visual-modes/waybar/styles/"* "$HOME/.config/waybar/" || warn "Visual modes waybar styles deployment failed"
    fi

    # Copy macOS dock configs
    if [ -d "$SCRIPT_DIR/visual-modes/dock" ]; then
        cp "$SCRIPT_DIR/visual-modes/dock/"* "$HOME/.config/waybar/" || warn "macOS dock config deployment failed"
    fi
    ...
    if [ -d "$SCRIPT_DIR/visual-modes/mako" ]; then
        cp "$SCRIPT_DIR/visual-modes/mako/"* "$HOME/.config/mako/" || warn "Mako visual mode configs failed"
    fi
    ...
    cp "$SCRIPT_DIR/config-data/rofi/config.rasi" "$HOME/.config/rofi/config.rasi" || warn "Rofi config deployment failed"
    ...
    cp "$SCRIPT_DIR/config-data/btop/btop.conf" "$HOME/.config/btop/btop.conf" || warn "btop config deployment failed"
    cp "$SCRIPT_DIR/config-data/btop/themes/apollo-hacker.theme" "$HOME/.config/btop/themes/apollo-hacker.theme" || warn "btop theme deployment failed"

    # Deploy Hyprlock configuration (for Niri lock screen)
    if [[ "$INSTALL_NIRI" == true ]]; then
        log "Deploying Hyprlock configuration..."
        mkdir -p "$HOME/.config/hypr/hyprlock"
        if [ -d "$SCRIPT_DIR/visual-modes/hyprlock" ]; then
            cp "$SCRIPT_DIR/visual-modes/hyprlock/hyprlock.conf" "$HOME/.config/hypr/" 2>/dev/null || warn "Hyprlock config failed"
            cp "$SCRIPT_DIR/visual-modes/hyprlock/colors.conf" "$HOME/.config/hypr/hyprlock/" 2>/dev/null || warn "Hyprlock colors failed"
            cp "$SCRIPT_DIR/visual-modes/hyprlock/"*.sh "$HOME/.config/hypr/hyprlock/" 2>/dev/null || warn "Hyprlock scripts failed"
            chmod +x "$HOME/.config/hypr/hyprlock/"*.sh 2>/dev/null
            log "Hyprlock configuration deployed ✓"
        else
            warn "Hyprlock configuration not found in visual-modes"
        fi
    fi
...
install_audio_system() {
    ...
    if [ -x "$EDGE_TTS" ]; then
        ...
        if [ -f "$SCRIPT_DIR/sounds/sleep.mp3" ]; then
            cp "$SCRIPT_DIR/sounds/sleep.mp3" "$SOUNDS_DIR/"
            cp "$SCRIPT_DIR/sounds/wake.mp3" "$SOUNDS_DIR/"
...
install_voice_control() {
    ...
    if [ -f "$SCRIPT_DIR/scripts/apollo-wake-listener.py" ]; then
        cp "$SCRIPT_DIR/scripts/apollo-wake-listener.py" "$HOME/.local/bin/" || warn "Failed to copy wake listener"
    ...
    if [ -f "$SCRIPT_DIR/scripts/apollo-speak.sh" ]; then
        cp "$SCRIPT_DIR/scripts/apollo-speak.sh" "$HOME/.local/bin/" || warn "Failed to copy apollo-speak"
    ...
    for script in voice-input voice-input-notification voice-input-visualizer; do
        if [ -f "$SCRIPT_DIR/scripts/$script" ]; then
            cp "$SCRIPT_DIR/scripts/$script" "$HOME/.local/bin/" || warn "Failed to copy $script"
            chmod +x "$HOME/.local/bin/$script"
        fi
    done

    # Copy Right Ctrl Push-to-Talk script
    if [ -f "$SCRIPT_DIR/scripts/apollo-os-rightctrl-voice.py" ]; then
        cp "$SCRIPT_DIR/scripts/apollo-os-rightctrl-voice.py" "$HOME/.local/bin/" || warn "Failed to copy rightctrl voice"
    ...
    if [ -d "$SCRIPT_DIR/visual-modes/sounds" ]; then
        cp "$SCRIPT_DIR/visual-modes/sounds/"*.wav "$HOME/.local/share/apollo-os/sounds/" 2>/dev/null || true
    fi
...
configure_login_manager() {
    ...
    sudo cp "$SCRIPT_DIR/systemd/logind-delay.conf" /etc/systemd/logind.conf.d/apollo-os-delay.conf 2>/dev/null || true
    ...
    if [[ "$INSTALL_HYPRLAND" == true ]]; then
        if [[ -f "$SCRIPT_DIR/config-data/wayland-sessions/apollo-os-glass.desktop" ]]; then
            sudo cp "$SCRIPT_DIR/config-data/wayland-sessions/apollo-os-glass.desktop" \
    ...
    if [[ "$INSTALL_NIRI" == true ]]; then
        if [[ -f "$SCRIPT_DIR/config-data/wayland-sessions/apollo-os-orbit.desktop" ]]; then
            sudo cp "$SCRIPT_DIR/config-data/wayland-sessions/apollo-os-orbit.desktop" \
...
SHOWTIME_EOF
    ...
    if [ -d "$SCRIPT_DIR" ] && [ "$SCRIPT_DIR" != "$HOME" ]; then
        ...
        rm -rf "$SCRIPT_DIR/systemd" 2>/dev/null || true
        rm -rf "$SCRIPT_DIR/apollo-os-professional" 2>/dev/null || true
